...
def plot_certsize_vs_handshake_throughput():
    ...
    # Labels mit leichtem X-Offset, damit sie nicht über den Punkten stehen
    x_offset_factor = 1.03  # Faktor für Rechtsverschiebung, anpassen nach Bedarf
    y_offset = 0  # optional, kann kleine vertikale Verschiebung hinzufügen

    # PQ
    for _, row in df_pq.iterrows():
        plt.text(
            row["Cert Size (KB)"] * x_offset_factor,
            row["Median Handshake Throughput"] + y_offset,
            row["Signing Algorithm"],
            fontsize=9,
            ha="left",  # Label rechts vom Punkt
            va="center" # vertikal mittig am Punkt
        )
    # Classic    
